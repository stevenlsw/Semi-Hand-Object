...
class HONet(nn.Module):
    def __init__(self, roi_res=32, joint_nb=21, stacks=1, channels=256, blocks=1,
        ...
        # CR blocks
        self.attention_HO = AttentionBlock(in_channels=channels, inter_channels=channels//2, bn_layer=True)
        ...
        if self.training:
            ...
            if mano_params is not None:
                ...
                for key, val in mano_losses.items():
                    losses[key] = val
            elif mano_params is None and joints_xyz is not None:
                gt_mano_results = {"joints3d": joints_xyz}
                mano_joint_loss, mano_joint_losses = self.mano_joint_loss.compute_loss(pred_mano_results,
                                                                                       gt_mano_results)
                for key, val in mano_joint_losses.items():
                    losses[key] = val
                total_loss += mano_joint_loss
